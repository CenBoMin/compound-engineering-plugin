...
# Git Commit, Push, and PR (Zed)

## Mode

- **Description-only** -- user wants _just_ a description ("write/draft a PR description", "describe this PR", or pasted a PR URL/number alone). Run Step 4 only; print the result. Apply only if the user asks. If a PR ref was pasted, pass it to Step 4 so Pre-A resolves the right range.
...
- Use Zed's blocking question tool for branch creation decisions and PR application confirmations, with numbered options.
- Avoid silent defaults on branch creation; always surface when the base freshness was not verified.
...
- **Capture now** -- use the relevant skill to capture evidence. It returns `Tier`, `Description`, `URL`, `Path`. Exactly one of `URL`/`Path` contains a real value; the other is `"none"`. If `URL`, splice as a `## Demo` section. If `Path` (user chose local save), note in the body that a demo was recorded but is not embedded. If skipped, proceed without evidence.
...
<the composed body markdown goes here>
__CE_PR_BODY_END__
