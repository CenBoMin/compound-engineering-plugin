...
Spawn the three reviewer agents below via `spawn_agent`. Dispatch all three in a single message so they run concurrently. Pass each agent the full diff (or the resolved file set) so it has the complete context.
...
- Use `spawn_agent` for the three parallel reviewer agents. Dispatch all three in the same message so they run concurrently.
- Prompts must be self-contained. Include the full diff or resolved file set in each subagent's prompt.
